...
void BLETx::setup() { ESP_LOGI(TAG, "ready"); }

void BLETx::loop() {
    const uint32_t now = millis();
    if (cool_) {
        if (now - t1_ < COOL_MS) return;
        cool_ = false;
    }
    if (adv_ && now - t0_ >= ADV_MS) {
        esp_ble_gap_stop_advertising();
        adv_ = false;
        t1_ = now;
        cool_ = true;
        if (!q_.empty()) { t2_ = now + GAP_MS; wait_ = true; }
    }
...
void BLETx::send_hex(const std::string &hex) {
    ...
    while (true) {
        size_t p = hex.find('|', s);
        auto pkt = (p == std::string::npos) ? hex.substr(s) : hex.substr(s, p - s);
        if (!pkt.empty()) q_.push_back(pkt);
...
void BLETx::raw_(const std::string &h) {
    ...
    t0_ = millis();
    adv_ = true;
    ESP_LOGI(TAG, "TX %u bytes", (unsigned)d.size());
}
...
std::vector<uint8_t> BLETx::bytes_(const std::string &h) {
    ...
    for (char c : h) {
        ...
        pair[n++] = c;
        if (n == 2) { d.push_back((uint8_t)strtol(pair, nullptr, 16)); n = 0; }
    }
    return d;
}
